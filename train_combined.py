...
class GPT(nn.Module):
    ...
    def _run_blocks(self, x: Tensor, x0: Tensor, causal: bool = True) -> Tensor:
        """Run U-Net encoder/decoder with skip connections."""
        skips = []
    ...
    def _ce_loss(self, x: Tensor, targets: Tensor, reduction: str = "mean", temperature: float = 1.0) -> Tensor:
        """Compute cross-entropy + Z-loss from hidden states."""
        logits = self._softcap(self._compute_logits(x))
        if temperature != 1.0:
